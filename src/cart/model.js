...
const getCartData = async (req, client) => {
    try {
        const response = await client.query(`SELECT * FROM cart WHERE "userid" = $1`, [req.params.id])
        if (response) {
...
const deleteCartAllData = async (req, client) => {
    const response = await client.query(`DELETE FROM cart WHERE "userid" = $1`, [req.params.id])
    try {
        if (response.rowCount > 0) {
            return { error: false, message: "cart data deleted successfully" };
        } else {
...
const getCartDetail = async (req, client) => {
    try {
        const response = await client.query(`SELECT * FROM cart WHERE "cartid" = $1`, [req.params.id])
        if (response) {
            return { error: false, data: response.rows, message: "cart read successfully" };
        } else {
            return { error: true, message: "cart not read successfully" };
